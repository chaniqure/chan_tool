...
func (t *LocalTime) SetString(str string) *LocalTime {
	if t != nil {
		...
		nowDate, err := time.ParseInLocation("2006-01-02", str, time.Local)
		if err == nil {
			*t = LocalTime{Time: nowDate}
		}
		nowMonth, err := time.ParseInLocation("2006-01", str, time.Local)
		if err == nil {
...
func (t *LocalTime) SetHourAndMinuteString(str string) *LocalTime {
	if t != nil {
		...
		dateStart := t.TodayStart().Format("2006-01-02")
